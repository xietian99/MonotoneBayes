...
// calculated our HS prior model with covariates
// the link function could be specified as "logit", ( future "log-log", "probit" )
//
...
transformed parameters {
  ...
  for (i in 1:N){
    ...
    base[i] = logit(base[i]); //?
    // adding covariates Z
    base[i] = base[i] + Z[i] * Gamma;
    // expit
    p[i] = inv_logit(base[i]); //?
  }
